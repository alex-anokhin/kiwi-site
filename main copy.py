...
secret_path = "/run/secrets/CONTACT_API"
if os.path.exists(secret_path):
    with open(secret_path, "r") as file:
        os.environ["CONTACT_API"] = file.read().strip()
...
def solution():
    ...
    if request.method == 'POST':
        ...
        response.set_cookie('user_id', user_id, max_age=30 * 24 * 60 * 60)  # Set a cookie to remember the user's ID
        return response
    else:
        # Render index page for GET requests
        user_id = request.cookies.get('user_id', str(uuid.uuid4()))
